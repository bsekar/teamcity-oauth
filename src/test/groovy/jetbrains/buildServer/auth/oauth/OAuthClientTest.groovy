...
class OAuthClientTest extends Specification {
    ...
    def "should fetch user data"() {
        ...
        server.enqueue(new MockResponse().setBody(JSONValue.toJSONString([id: 'user', name: 'userName', email: 'email', groups: ['pod-']])))
        expect:
        client.getUserData(token) == new OAuthUser('user', 'userName', 'email', Arrays.asList("pod-"))
        def req = server.takeRequest()
        req.method == 'GET'
        req.path == '/user'
